...
gather tax at B=1 and P3-3a stays suspended. The verdict below scales
the threshold linearly to the smallest swept Sk so long-context sweeps
stay comparable.

Default shape: Qwen3.8-27B full-attention layer, per-rank under TP=2 —
Hq=12, Hkv=2, D=256 (Hq is only used for the Q-side block; the gather
itself is Hkv/D-bound). The original 35B-A3B shape (Hq=16) is restored
with BENCH_G_HQ=16.

Env knobs:
  G_SK        comma list of Sk to sweep (default 65536,98304,122880 —
              the PROFILE-k4-step-ledger.md production contexts)
  BENCH_G_HQ / BENCH_G_HKV / BENCH_G_D / BENCH_G_BLOCK

Run in the gfx906 vLLM image with the repo source-mounted:
  python3 -u bench_gfx906_fa_gather_qwen27.py
"""
import torch
...
import os as _os
Hq = int(_os.environ.get("BENCH_G_HQ", "12"))
Hkv = int(_os.environ.get("BENCH_G_HKV", "2"))
D = int(_os.environ.get("BENCH_G_D", "256"))
BLOCK = int(_os.environ.get("BENCH_G_BLOCK", "16"))
BPR = (D // 32) * 34  # uint8 per Q8 row
SK_LIST = [int(x) for x in _os.environ.get("G_SK", "65536,98304,122880").split(",")]
HBM_BW = 798e9  # P3-0 Q1: measured MI50 HBM read BW
GATE_US = 80.0  # plan §4: gather > ~80 us/layer -> P3-3a stays suspended


def time_us(fn, warmup=None, iters=None):
    import math
    if warmup is None: warmup=max(3, 20 - max(SK_LIST)//65536)
    if iters is None: iters=max(5, 200 - max(SK_LIST)//1024)
...
def main():
    ...
    # -------- gate verdict (scaled to the smallest swept Sk) --------
    sk_g = min(SK_LIST)
    g = gather[sk_g]
    gate_scaled = GATE_US * sk_g / 2816
    print(f"\nGATE (plan §4, P3-3a): gather @ Sk={sk_g} = {g:.1f} us/layer "
          f"vs ~{gate_scaled:.0f} us scaled threshold ({GATE_US:.0f} us @2816)")
    print(f"  + Q-side {side:.1f} us/layer -> combined {g + side:.1f} us/layer")
    if g <= gate_scaled:
        print("  => gather <= scaled threshold: P3-3a may RESUME (M1 work, per plan)")
    else:
        print("  => gather > scaled threshold: P3-3a stays SUSPENDED")
