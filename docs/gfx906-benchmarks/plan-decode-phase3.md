...
Status: DRAFT v1 (evidence from graph-mode + eager attribution profiles,
...
LLGemm1 path as their siblings. Expectation: 1.6 → ~0.3 ms/step.
...
Compare against llama.cpp's attention kernel from the P3-0 trace. If its
decode attention is materially faster at batch=1/seq~500, study the layout
(KV page size, head split) before writing anything. **Gate:** P3-0 shows a
gap ≥2×; otherwise defer (our FA prefill advantage must not regress —
bench both phases).

### P3-4 — elementwise/norm fusion via inductor pass_config (~1–2 ms/step)
Enable the applicable `pass_config` fusions for this model and measure.
Zero kernel code, but numerics-sensitive: full correctness test + sanity
generation required. **Gate:** P3-0 Q5 shows the pile is fusion-eligible;
any output diff beyond fp rounding → revert.
