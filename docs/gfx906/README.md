...
| env | default | effect |
|---|---|---|
| `GFX906_FA_LEGACY` | 1 | fp16 KV cache + in-kernel Q8 quantize (validated serving default); `0` = Q8 pre-quantized at KV write into a side **view aliased into the fp16 K half** — zero extra KV memory, COW-safe (page copies move the Q8 bytes); attention reads the Q8 directly instead of re-quantizing per read. 2026-08-27: 46/46 suite + default-config and prefix-cache smokes clean. **Orthogonal to the read pattern** (`GFX906_FA_DIRECT_PAGED*`): direct-paged is LEGACY=0-only (round-7 erratum); LEGACY=0's distinct contribution is the Q8 read (no repeated inline quantize). **TP=2 bake executed 2026-08-28 (roadmap M5, boot M): LEGACY=0 was SLOWER than the LEGACY=1 control at every point** (B=1 decode −2.5…−3.7 % @2k/8k; B=4 @2k aggregate −27…−31 %; prefill wash). **Round 10 (same day, M6 Part B): the B=4 loss was direct-paged's strided Q8-slice reads — rerouting LEGACY=0 B≥2 to the fused-Q8 gather (`GFX906_FA_DIRECT_PAGED_Q8=0`, now the default) recovered B=4 @2k to 35.7 → 46.3 t/s (parity with the 46.7 LEGACY=1 control, B=1/prefill unchanged); direct-paged is opt-in (=1) with no measured advantage. Mechanism (corrected 2026-08-28 by ISA measurement, `DEVLOG-fa-attention.md` 2026-08-28 entry): NOT an int8-compute gap — `v_dot4_i32_i8` is full-rate on gfx906 (4 int8 MAC/cyc, 2× packed fp16) and both arms share the same dot; the deficit is the **Q8 alias read layout** (136 B slices inside 256-B row strides, 34-B q8_0 block strides → sector waste; direct-paged amplifies it at B≥2) and the decode path is gather-HBM-bound so the leaner read never surfaces. **Default stays `1`**; `0` remains an experimental opt-in (zero-extra-KV-memory alias, COW-safe) — reopenable via the M6 layout fix + serving A/B |
