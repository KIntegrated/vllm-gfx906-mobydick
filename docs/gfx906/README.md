...
| **Qwen3.8-27B-AWQ-INT4** (dense) | **fully functional (TP=1 + TP=2)** | **28.62** @4k TP=1 (28.91 later run; holds at 131k ctx) / **28.83** @4k + **32.79** @131k TP=2 | — | `--dtype float16` required (auto-bf16→fp16 fallback landed); MTP k=2 **41.41** (TP=1, 1.513×) / **42.63** (TP=2, record); TP=2 needs the official amdgpu DKMS driver + trimmed capture `[1,2,3,4]`; 445k-token KV pool (256k bootable max_model_len) vs 131k at TP=1; records: `DEVLOG-tp2-dense.md` S1–S8 + `DEVLOG-masked-fa.md` |
