...
| `GFX906_FA_WINDOW_CLIP` | 1 | Phase C: windowed decode (direct-paged, B≥2) starts the KV scan at the window start (kernel floors to the KV-tile boundary when the gained keys are mask-killed, so the result is bit-identical to the masked full scan); `0` kill switch (numerically identical, slower). Fires on the direct-paged **decode dispatch** (auto: B≥2, Sq≤16 — both LEGACY modes; the LEGACY=1 variant is not separately e2e-gated — verification gap); e2e gate +3.6% @ pp8192/B=2 (DEVLOG-muse-glimmer 2026-08-27 round 3) |
