...
| **cyankiwi/Muse-Glimmer-30B-AWQ-INT4** (dense hybrid: GDN + full + sliding-2048, CT W4A16) | **supported (2026-08-27, `feat/muse-glimmer` — not yet in main; TP=1 + TP=2)** | TP=1 in-process: **27.90** @B=1 all-CUSTOM window FA (1.59× vs hybrid 17.54) · **20.53** @B=4 (1.23× vs 16.75). TP=2 ngram n=5 serving (repetitive filler, **100 % acceptance ceiling**): **114.6** @2k/256 · 112.0 @2k/512 · **79.1** @8k/256 · 79.2 @8k/512 · **57.0** @16k/256 · 56.8 @16k/512 · B=4 @2k/256: **45.3** aggregate (~11.3/req); real-prompt checks ~11.5/req (1–4 parallel) | TP=2 warm: **542** @2k · **491** @8k · **438** @16k (first sample 498 @2k); TP=1 gates: pp2048 tg256, 4 samples, prefix cache off | **Working TP=2 example** (boot K, 2026-08-27): `HIP_VISIBLE_DEVICES=0,1 FLASH_ATTENTION_TRITON_AMD_ENABLE=TRUE vllm serve <snap> --served-model-name Muse-Glimmer-30B --tensor-parallel-size 2 --dtype float16 --max-model-len 131072 --max-num-seqs 4 --max-num-batched-tokens 2048 --kv-cache-memory-bytes 6442450944 --speculative-config '{"method":"ngram","num_speculative_tokens":5,"prompt_lookup_max":2}' --compilation-config '{"cudagraph_capture_sizes":[6,12,18,24]}' --enable-auto-tool-choice --tool-call-parser muse_glimmer --reasoning-parser muse_glimmer --generation-config auto` — pool 848–904k tok (6.5–7× the 128k max), weights 12.7 GiB/GPU, graphs ~0.9 GiB; 256k ctx: add `VLLM_ALLOW_LONG_MAX_MODEL_LEN=1` (boot J, 1.36M-tok pool). **bt4096 OOMs the first 4096-chunk prefill even with this cap** — the compiled prefill transient scales ~linearly with the chunk (bt1024 OK / bt2048 needs ~4–7 GiB margin / bt4096 >10.6 GiB at TP=2; the startup profile measures the *eager* peak, not the piecewise-compiled runtime) → **bt2048 required**; the byte cap is also required (at util 0.82 the pool is sized from that profile — the engine logs the correction itself: 7.59 GiB fits the line, 9.02 was allocated). The filler is 100 % ngram-accepted (acceptance length 6.0) — decode numbers are a ceiling, not real-text. 52 layers (13 full + 39 sliding window-2048): sliding-window Q8 FA (window arg, both kernel copies) + direct-paged split-K + Phase C clip (`GFX906_FA_LEGACY=0`); TP=1 bench gates need an explicit 0.75 GiB KV cap + bt1024; `muse_glimmer` tool/reasoning parsers; records: `DEVLOG-muse-glimmer.md`, `degradation*.md` boot J/K |
...
| env | default | effect |
|---|---|---|
| `GFX906_FA_LEGACY` | 1 | fp16 KV cache + in-kernel Q8 quantize (validated serving default); `0` = Q8 side **view aliased into the fp16 K half** — zero extra KV memory, COW-safe (page copies move the Q8 bytes), 2026-08-27: 46/46 suite + default-config and prefix-cache smokes clean. Unlocks the direct-paged read path (Phase C clip + batch-aware KVSPLIT) |
...
| `GFX906_FA_KVSPLIT` | gather 16; direct-paged `clamp(16/B,2,8)` | decode KV-split factor (one knob, two paths — an exported value pins BOTH; safe explicit values: gather `16`, direct-paged `8` at B≤2 / `2` at B≥4); `1` disables; `0` is NOT the unset default (it clamps to 1) |
...
| `GFX906_FA_WINDOW_CLIP` | 1 | Phase C: windowed decode (direct-paged, B≥2) starts the KV scan at the window start (kernel floors to the KV-tile boundary when the gained keys are mask-killed, so the result is bit-identical to the masked full scan); `0` kill switch (numerically identical, slower). Only reachable via the direct-paged path (`GFX906_FA_LEGACY=0`); e2e gate +3.6% @ pp8192/B=2 (DEVLOG-muse-glimmer 2026-08-27 round 3) |
