...
| **cyankiwi/Muse-Glimmer-30B-AWQ-INT4** (dense hybrid: GDN + full + sliding-2048, CT W4A16) | **supported (2026-08-27, `feat/muse-glimmer` — not yet in main)** | **27.90** @B=1 all-CUSTOM window FA (1.59× vs hybrid 17.54) · **20.53** @B=4 (1.23× vs 16.75) | pp2048 tg256, 4 samples, prefix cache off | 52 layers (13 GDN / full / sliding window-2048): sliding-window Q8 FA (window arg, both kernel copies) + direct-paged split-K + Phase C clip (`GFX906_FA_LEGACY=0`); serving default config clean (util 0.93, 95k pool, maxlen 8192); bench gates need an explicit 0.75 GiB KV cap + bt1024 (default pool OOMs the inductor prefill peak on the first 2048 chunk); qwen3_coder/qwen3 parsers; `DEVLOG-muse-glimmer.md` |
