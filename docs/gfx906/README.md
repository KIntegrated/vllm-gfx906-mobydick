...
| **Qwen3.8-27B-AWQ-INT4** (dense) | **fully functional (TP=1 + TP=2 + N=8)** | **28.62** @4k TP=1 / **28.83** @4k + **32.79** @131k TP=2 / 104.2 (N=8, W4 on) | — | `--dtype float16` required (auto-bf16→fp16 fallback landed); MTP k=2 **41.41** (TP=1, 1.513×) / **42.63** (TP=2, record); N=8 needs `--gpu-memory-utilization 0.90` (64 layers, FA KV 655 KB/token); TP=2 needs the official amdgpu DKMS driver + trimmed capture `[1,2,3,4]`; 445k-token KV pool — **256k context validated** (FA gather fix, 2026-08-24: the 131k ceiling lifted, `oom-256k-prefill.md` §9); non-deterministic at temp=0 (token-identity gates unusable); records: `DEVLOG-tp2-dense.md` S1–S8 + `DEVLOG-masked-fa.md` |
