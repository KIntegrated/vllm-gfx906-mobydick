...
| M6 Part A planar Q8 quants/scale repack closes the LEGACY=0 B=1 gap (flip re-open) | microbench hard stop-rule (ISA loader loads ≥2× AND step ≥2 %) | **DEAD-END** (flip question); code **NEUTRAL** — merged as loader hygiene 2026-08-29 (`02d197189f`), flip-question verdict unchanged | same-boot B=1 A/B PASS: slope 36.0→34.4 ns/token (−4.3/−4.8 %), @Sk=2176 83.6→79.1 us, bit-identical (maxerr equal at every Sk), merged suite 74/74 | ISA-verified loader 10→6 loads/tile-row = 1.67× < 2× (plan's ~17/block assumption was wrong — compiler already 4×8-B + 2-B); standalone B=1 step −2.4 % (disjoint bands) both LEGACY modes; bit-identical (64/64). B=1 gap is elsewhere (write path / Q-side / gather traffic) | MG, plan_fa_part_A.md |
