...
| 2026-08-23 05:18:29 | 0 | HW | 27B mtp2 canary (W2 session) SIGABRT rc=134; `Fence fallback timer expired on ring comp_1.0.0` → `GPU reset(1) succeeded` — first HW reset this boot. Pre-wedge: amdsmi broken since boot (08-22 20:05Z); 35B-MoE mtp2 smoke ~39.5 t/s decode (vs ~81 baseline) + baseline greedy FP non-reproducible 04:5x–05:1xZ (unexplained — boundary or early degradation, unknown) | recovered via driver reset; canary re-run pending |
