...
class CompressedTensorsWNA16MoEMethod(CompressedTensorsMoEMethod):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # symmetric quant. The gfx906 kernel does not: for symmetric (no-zp)
        # weights the repack returns None, the op is called with an empty zp,
        # and the kernel inlines the constant zero point 8 — so symmetric
        # gfx906 layers carry no zero_point parameters. Upstream's EMULATION
        # backend bakes ZP into the dequantized bf16 weights (ZP is None) and
